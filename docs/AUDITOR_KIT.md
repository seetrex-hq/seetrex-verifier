...
<!-- doc-revision:begin -->
**Document revision: `2026-09-03-recapture-0-3-7-page`.** What this revision
IS: the capture of ROUTE F — section 2.6, the offline browser page — against
the release as PUBLISHED. The page, its list and its signature were downloaded
from the release of `seetrex-verifier-v0.3.7` and both of that route's commands
were run over them on 2026-09-03, and the tag list of section 6 was re-verified
from a fresh clone the same day. Nothing else was re-measured, and every other
dated capture in this document keeps the day it already carried.
If your copy lacks a
...
**Result on 2026-09-03 against the `seetrex-verifier-v0.3.7` release: the page
is there and both commands pass.** That release carries one; the revision
before this one could not say which release did, because none did yet, and the
rule above — a release may carry it or not — is why this paragraph names a
release instead of the route. The three files were downloaded with exactly the
three commands of the fence above, `<version>` read as `0.3.7`, into an empty
directory, and the key of step 1 was imported into a keyring of its own:

```
$ gpg --verify seetrex-verifier-offline.SHA256SUMS.asc seetrex-verifier-offline.SHA256SUMS
gpg: Signature made Thu Sep  3 09:00:07 2026
gpg:                using EDDSA key F028DE16D3B2AA440FE26F05CECC557729596616
gpg: Good signature from "Seetrex Compliance Release Signing <release@seetrex.com>" [unknown]
gpg: WARNING: This key is not certified with a trusted signature!
gpg:          There is no indication that the signature belongs to the owner.
Primary key fingerprint: F028 DE16 D3B2 AA44 0FE2  6F05 CECC 5577 2959 6616
$ sha256sum --ignore-missing -c seetrex-verifier-offline.SHA256SUMS
seetrex-verifier-offline.html: OK
```

Six lines of that block need reading the way section 2.2 reads them, and for
the same reasons. `Signature made` is rendered in the READER's own local zone:
the capture above was taken at UTC+02:00, so the instant it names is
2026-09-03T07:00:07Z. The `WARNING` pair is *expected* — it says only that the
key has not been certified in your own web of trust, and the out-of-band
fingerprint comparison of section 2.3 is the check that replaces it. What
decides the first command is the `Good signature` line together with a printed
primary key fingerprint equal to the pinned one, character for character; what
decides the second is one `OK` line per file the list covers.

**This document still names no digest of the page**, and that is the same rule
section 2.4 states for the executables one route over. A digest pasted here
would be true of exactly this release and false from the next one on, and the
recapture that refreshed it is one an auditor cannot verify happened. What
anchors these bytes is the KEY that signs the list — fingerprint above, checked
out of band per section 2.3 — and the list you verified is the one that covers
